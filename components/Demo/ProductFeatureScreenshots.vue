    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6">
      ...
      <div id="tabs" class="lg:grid grid-cols-12 gap-6">
        ...
        <div class="col-span-4">
          <div class="tabs">
            <div v-for="(item, index) in data.features" :key="index" class="block mb-6 tab-content-wrap cursor-pointer border border-gray-f0 rounded-2xl p-8 duration-300" :class="activeItem == item.image.url ? 'bg-blue-500' : '' " @click="activeItem = item.image.url">
              <h3 class="font-medium text-2xl mb-4 duration-300" :class="activeItem == item.image.url ? 'text-white' : 'textdark'">
                {{ item.title }}
              </h3>
              <p class="test-base font-light duration-300" :class="activeItem == item.image.url ? 'text-white opacity-80' : 'text-dark-gray'">
                {{ item.description }}
              </p>
            </div>
          </div>
